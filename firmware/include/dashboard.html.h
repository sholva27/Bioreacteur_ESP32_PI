...
<body>
  ...
  <script>
    ...
    function provideBtnFeedback(btnId, successText) {
    ...
    function updateSettings(extra = {}, btnId = 'save-btn') {
      ...
      return fetch('/settings').then(r => r.json()).then(data => {
        ...
        return fetch('/set', {
          ...
          if (btnId) provideBtnFeedback(btnId, 'Saved!');
        });
      }).catch(err => {
        if (btnId) {
          const btn = document.getElementById(btnId);
    ...
    function togglePump(pump) {
      ...
      if (btnId) document.getElementById(btnId).innerText = 'Feeding...';
      fetch("/pump?type=" + pump).then(() => {
        if (btnId) provideBtnFeedback(btnId, 'Fed!');
      });
    }
