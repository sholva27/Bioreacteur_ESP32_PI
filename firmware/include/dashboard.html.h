...
<body>
  ...
  <div class="card">
    ...
    <button id="feed-btn" onclick="togglePump('nutrient')" aria-label="Run nutrient pump manually">Manual Feed</button>
  </div>
  ...
  <div class="card">
    ...
    <button id="od-btn" onclick="calibrateODZero()" aria-label="Set current OD reading as zero baseline">Set OD Blank (Zero)</button>
    <hr>
  ...
  <script>
    ...
    function calibratePH(value) {
       ...
       const done = provideBtnFeedback(btnId, 'Calibrating...');
       fetch('/data').then(r => r.json()).then(data => {
          if (value == 7.0) {
            lastPh7V = data.ph_v;
            alert("pH 7.0 set to " + lastPh7V + "V. Now place in pH 4.0 and calibrate.");
            updateSettings({phOffset: -lastPh7V}).finally(() => done());
          } else if (value == 4.0) {
            lastPh4V = data.ph_v;
            var newSlope = 3.0 / (lastPh7V - lastPh4V);
            var newOffset = 0 - (lastPh7V * newSlope);
            updateSettings({phSlope: newSlope, phOffset: newOffset}).finally(() => {
              done(Promise.resolve().then(() => {
                const btn = document.getElementById(btnId);
                if (btn) btn.innerText = 'Calibrated!';
              }));
    ...
    function calibrateODZero() {
       const done = provideBtnFeedback('od-btn', 'Setting...');
       fetch('/data').then(r => r.json()).then(data => {
          updateSettings({odZero: data.od_v}).finally(() => {
            done(Promise.resolve().then(() => {
              const btn = document.getElementById('od-btn');
              if (btn) btn.innerText = 'Set!';
            }));
    ...
    function updateSettings(extra = {}) {
      const isManualSave = Object.keys(extra).length === 0;
      const done = isManualSave ? provideBtnFeedback('save-btn', 'Saving...') : null;
    ...
    function togglePump(pump) {
      const done = (pump === 'nutrient') ? provideBtnFeedback('feed-btn', 'Feeding...') : null;
      const p = fetch("/pump?type=" + pump);
      if (done) done(p);
    }

    function runCalibrationPump() {
      const done = provideBtnFeedback('cal-run-btn', 'Running...', 60000);
    ...
    function savePumpCal() {
      const done = provideBtnFeedback('cal-save-btn', 'Saving...');
      var pump = document.getElementById('cal-pump-select').value;
    ...
    function provideBtnFeedback(btnId, statusText, duration = 2000) {
      const btn = document.getElementById(btnId);
      if (!btn) return;
