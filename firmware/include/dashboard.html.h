...
<body>
  ...
  <div class="card">
    ...
    <button id="download-btn" class="btn-download" onclick="window.location.href='/download_log'" aria-label="Download log">Download Log</button>
    <button id="feed-btn" onclick="togglePump('nutrient')" aria-label="Manual feed">Manual Feed</button>
  </div>

  <div class="card">
    <h2>Calibration</h2>
    <p>pH (Current Volts: <span id="ph-v">--</span>)</p>
    <button id="ph7-btn" onclick="calibratePH(7.0)" aria-label="Calibrate pH 7">Calibrate pH 7.0</button>
    <button id="ph4-btn" onclick="calibratePH(4.0)" aria-label="Calibrate pH 4">Calibrate pH 4.0</button>
    <p>OD (Current Volts: <span id="od-v">--</span>)</p>
    <button id="od-btn" onclick="calibrateODZero()" aria-label="Zero OD">Set OD Blank (Zero)</button>
    <hr>
    <h3>Pump Calibration</h3>
    <label for="cal-pump-select">Pump:</label>
    ...
    <button id="cal-run-btn" onclick="runCalibrationPump()" aria-label="Run pump">Run for 60s</button>
    <br>
    <label for="cal-vol">Vol (mL):</label> <input type="number" id="cal-vol" step="0.1">
    <button id="cal-save-btn" onclick="savePumpCal()" aria-label="Save rate">Save Flow Rate</button>
  </div>
  ...
  <script>
    ...
    function provideBtnFeedback(btnId, interimText, successText) {
      const btn = document.getElementById(btnId);
      if (!btn) return () => {};
      const orig = btn.innerHTML;
      btn.innerHTML = interimText; btn.disabled = true;
      return () => {
    ...
    function updateSettings(extra = {}, btnId = 'save-btn') {
      const resolve = btnId ? provideBtnFeedback(btnId, "Saving...", "Saved!") : () => {};
      fetch('/settings').then(r => r.json()).then(data => {
        const s = data;
        ...
        .then(() => resolve()).catch(e => { if(btnId) { document.getElementById(btnId).innerHTML = "Error!"; setTimeout(() => { document.getElementById(btnId).innerHTML = "Save Settings"; document.getElementById(btnId).disabled = false; }, 2000); } throw e; });
    ...
    function togglePump(pump) {
      const res = provideBtnFeedback('feed-btn', "Feeding...", "Fed!");
      fetch("/pump?type=" + pump).then(() => res()).catch(() => res());
    }

    function runCalibrationPump() {
      const res = provideBtnFeedback('cal-run-btn', "Running...", "Done!");
      fetch("/pump?type=" + document.getElementById('cal-pump-select').value + "&duration=60000")
      .then(() => { alert("Pump running for 60s."); res(); }).catch(() => res());
    }

    function savePumpCal() {
      const res = provideBtnFeedback('cal-save-btn', "Saving...", "Saved!");
      const fr = document.getElementById('cal-vol').value / 60.0;
      alert("Flow rate: " + fr.toFixed(4) + " mL/s");
      res();
    }
