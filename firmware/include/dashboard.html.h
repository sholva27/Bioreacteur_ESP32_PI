...
<body>
  ...
  <div class="card">
    ...
    <button class="btn-download" onclick="window.location.href='/download_log'" aria-label="Download experiment log as CSV">Download Log</button>
  ...
  <div class="card">
    ...
    <button id="ph7-btn" onclick="calibratePH(7.0)" aria-label="Calibrate pH probe with 7.0 buffer">Calibrate pH 7.0</button>
    <button id="ph4-btn" onclick="calibratePH(4.0)" aria-label="Calibrate pH probe with 4.0 buffer">Calibrate pH 4.0</button>
    <p>OD (Current Volts: <span id="od-v">--</span>)</p>
    <button id="od-btn" onclick="calibrateODZero()" aria-label="Set optical density zero point using blank sample">Set OD Blank (Zero)</button>
    <hr>
  ...
  <script>
    ...
    function calibratePH(value) {
       ...
       fetch('/data').then(r => r.json()).then(data => {
          if (value == 7.0) {
            ...
            updateSettings({phOffset: -lastPh7V}, btnId);
          } else if (value == 4.0) {
            lastPh4V = data.ph_v;
            var newSlope = 3.0 / (lastPh7V - lastPh4V);
            var newOffset = 0 - (lastPh7V * newSlope);
            updateSettings({phSlope: newSlope, phOffset: newOffset}, btnId);
    ...
    function calibrateODZero() {
       fetch('/data').then(r => r.json()).then(data => {
          updateSettings({odZero: data.od_v}, 'od-btn');
          alert("OD Blank set to " + data.od_v + "V");
       });
    }

    function updateSettings(extra = {}, btnId = 'save-btn') {
      const done = (btnId && !Object.keys(extra).length) ? provideBtnFeedback(btnId) : null;
      fetch('/settings').then(r => r.json()).then(data => {
        var settings = data;
    ...
    function togglePump(pump) {
      const done = provideBtnFeedback('feed-btn', 'Fed!');
      fetch("/pump?type=" + pump).then(() => done());
    }

    function runCalibrationPump() {
      var pump = document.getElementById('cal-pump-select').value;
      const done = provideBtnFeedback('cal-run-btn', 'Done!');
    ...
    function savePumpCal() {
      ...
      const done = provideBtnFeedback('cal-save-btn', 'Saved!');
